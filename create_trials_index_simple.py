...
def make_es_request(method, path, data=None):
    ...
    resp = None
    try:
        if method == "GET": resp = requests.get(url, auth=auth, headers=headers)
        elif method == "PUT": resp = requests.put(url, auth=auth, headers=headers, json=data)
        elif method == "HEAD": resp = requests.head(url, auth=auth, headers=headers)
    except Exception as e: logger.error(f"Request failed: {e}")
    return resp
